...
Minimal input/output interface for audio and video.

Features:
- Register/unregister video input callbacks
- Register/unregister audio input callbacks
- Register output handlers for video and audio
- Send video/audio outputs to registered output handlers
- Text input/output are explicitly blocked from model forwarding and return a rejection
...
Usage examples:
    def my_video_input(frame_bytes, meta):
        # handle incoming frame
        pass

    def my_video_sink(frame_bytes, meta):
        # send frame to display or encoder
        return True

    register_video_input(my_video_input)
    register_video_output(my_video_sink)
    send_video_output(b"...frame...", {"format": "rgb"})
...
_video_outputs = []  # list of callables(frame_bytes, meta) -> bool
_audio_outputs = []  # list of callables(chunk_bytes, meta) -> bool

# Single-module hooks per user request: SFE and Output
_sfe_module = None
_output_module = None
...
# --- Begin embedded classes from audioin.py, videoin.py, audioout.py ---
...
class AudioIn:
    ...
    """
    def __init__(self, device_index: int = None, sample_rate: int = 16000, frame_size: int = 1024, buffer_seconds: float = 2.0, feature_mode: bool = False, channels: int = 2):
...
class VideoIn:
    ...
    """
    def __init__(self, device_index: int = 0, target_feature_dim: int = 512, thumb_size: tuple = (32, 32)):
        self.device_index = device_index
...
class AudioOut:
    """Audio output playback using sounddevice (from audioout.py).
...
# Registration API for outputs (sinks)
def register_video_output(callback):
    """Register a video output handler (sink).

    The handler should accept (frame_bytes, meta) and return True on success,
    False on failure. Handlers are called synchronously in order of registration.
    """
    if not _is_callable(callback):
        return False
    if callback in _video_outputs:
        return True
    _video_outputs.append(callback)
    return True


def unregister_video_output(callback):
    try:
        _video_outputs.remove(callback)
        return True
    except ValueError:
        return False


def register_audio_output(callback):
    """Register an audio output handler (sink).

    Handler signature: handler(chunk_bytes, meta) -> bool
    """
    if not _is_callable(callback):
        return False
    if callback in _audio_outputs:
        return True
    _audio_outputs.append(callback)
    return True


def unregister_audio_output(callback):
    try:
        _audio_outputs.remove(callback)
        return True
    except ValueError:
        return False
...
# Sending outputs
def send_video_output(frame_bytes, meta=None):
    """Send a video frame to all registered video output handlers.

    Returns a dict with results and an overall success boolean:
        {"success": bool, "results": [bool, ...]}

    If no handlers are registered, returns {"success": False, "results": []}.
    """
    if meta is None:
        meta = {}
    results = []
    for h in list(_video_outputs):
        try:
            ok = bool(h(frame_bytes, meta))
        except Exception as e:
            print("video output handler error:", e)
            ok = False
        results.append(ok)
    return {"success": any(results), "results": results}


def send_audio_output(chunk_bytes, meta=None):
    """Send an audio chunk to all registered audio output handlers.
    ...
    if meta is None:
        meta = {}
    results = []
    for h in list(_audio_outputs):
        try:
            ok = bool(h(chunk_bytes, meta))
        except Exception as e:
            print("audio output handler error:", e)
            ok = False
        results.append(ok)
    return {"success": any(results), "results": results}
    ...
    if _sfe_module is not None:
        try:
            if hasattr(_sfe_module, 'get_features'):
                ...
                _last_sfe_features = _sfe_module.get_features(_last_audio_chunk, _last_video_frame)
        except Exception as e:
...
def set_output(module):
    """Set the output module that receives vocal synth buffers.

    The module is called when vocalsynth emits audio. inout will try common method names
    in order: play_chunk, play_audio, receive_audio, or if the module is callable it will be called.
    """
    global _output_module
...
def vocalsynth_emit(buffer, meta=None):
    """Forward a buffer produced by VocalSynth to the selected output module or registered sinks.

    buffer: numpy array (float64) or bytes
    """
    global _output_module
    sent = False
    if _output_module is not None:
        try:
            if hasattr(_output_module, 'play_chunk'):
                _output_module.play_chunk(buffer)
                sent = True
            elif hasattr(_output_module, 'play_audio'):
                _output_module.play_audio(buffer)
                sent = True
            elif hasattr(_output_module, 'receive_audio'):
                _output_module.receive_audio(buffer, meta)
                sent = True
            elif callable(_output_module):
                try:
                    _output_module(buffer, meta)
                    sent = True
                except Exception:
                    sent = False
        except Exception as e:
            print("Error forwarding to output module:", e)

    # Also send to any registered audio outputs for backwards compatibility
    if not sent:
        try:
            send_audio_output(buffer, meta)
        except Exception as e:
            print("Error sending to registered audio outputs:", e)

    return sent


# Create and register a default AudioOut as the final speaker sink
_default_audioout = None
try:
    _default_audioout = AudioOut(device_index=None)
    # Try to start if sounddevice available
    try:
        _default_audioout.start()
        set_output(_default_audioout)
        logger.info("inout: default AudioOut started and registered as output module.")
    except Exception as e:
        logger.warning(f"inout: default AudioOut failed to start: {e}")
        # still set as output so vocalsynth_emit will attempt to use it
        set_output(_default_audioout)
except Exception:
    _default_audioout = None
...
def text_output(target, text):
    """Attempt to send text output but block forwarding to model output path.

    Returns a dict with rejection info.
    """
    # Allow text output paths by default; callers decide whether to send to model output.
    return {"allowed": True, "payload": {"target": target, "text": text}}
...
__all__ = [
    ...
    "unregister_video_output",
    "register_audio_output",
    "unregister_audio_output",
    "send_video_output",
    "send_audio_output",
    "deliver_video_input",
    "deliver_audio_input",
