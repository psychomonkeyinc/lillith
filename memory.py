...
class MemoryFragment:
    ...
    def get_relevance(self, current_time: float, query_emotion: np.ndarray) -> float:
        time_factor = np.exp(-(current_time - self.last_access_time) / 3600)  # Decay over hours
        emotional_similarity = np.dot(self.emotional_context, query_emotion) / (np.linalg.norm(self.emotional_context) * np.linalg.norm(query_emotion) + 1e-6)
        access_factor = np.log1p(self.access_count)
        return time_factor * (0.7 * emotional_similarity + 0.3 * access_factor)

class MemoryConsolidation:
    def __init__(self, memory_dimension: int, internal_hidden_dim: int = 128):
        self.dimension = memory_dimension
        self.consolidation_network = Sequential(
            Linear(memory_dimension, internal_hidden_dim),
            Tanh(),
            Linear(internal_hidden_dim, memory_dimension)
        )
        self.emotional_weighting = np.ones(memory_dimension, dtype=np.float32)
        
    def update_dimensions(self, memory_dimension: int, internal_hidden_dim: int = 128):
...
class SensoryBuffer:
    ...
    def __init__(self, size: int = 100, sfe_feature_dim: int = 55):
        self.size = size
    ...
    def add(self, sfe_feature_vector: np.ndarray):
        ...
        if sfe_feature_vector.shape[0] != self.sfe_feature_dim:
            ...
            return
        self.buffer.append(sfe_feature_vector.astype(np.float32))
...
class WorkingMemory:
    ...
    def __init__(self, size: int = 10, cognitive_state_dim: int = 256): # Using V4 cognitive state dim
        self.size = size
        self.cognitive_state_dim = cognitive_state_dim
        self.canvas = np.zeros((size, cognitive_state_dim), dtype=np.float32) # V4 uses float32
    ...
    def update(self, cognitive_state_vector: np.ndarray):
        ...
        self.canvas[-1] = cognitive_state_vector[0, :self.cognitive_state_dim].astype(np.float32)

    def get_state(self) -> np.ndarray:
        return self.canvas.flatten().astype(np.float32)
    
    def clear(self):
        self.canvas = np.zeros((self.size, self.cognitive_state_dim), dtype=np.float32)

class AffectiveLedger:
    """A permanent, chronological log of emotionally significant events."""
    def __init__(self, max_memories: int = 2000, emotional_state_dim: int = 108, cognitive_state_dim: int = 256): # V4 dims
        self.max_memories = max_memories
        self.emotional_state_dim = emotional_state_dim
        self.cognitive_state_dim = cognitive_state_dim
        self.memories = deque(maxlen=max_memories)
        self.emotional_threshold = np.float32(0.6)
    ...
    def record_event(self, cognitive_state: np.ndarray, emotional_state: np.ndarray, timestamp: float):
        ...
        if emotional_charge > self.emotional_threshold:
            memory_entry = {
                "cognitive_state": cognitive_state.copy().astype(np.float32),
                "emotional_state": emotional_state.copy().astype(np.float32),
                "charge": emotional_charge.astype(np.float32),
...
class AssociativeMemory:
    ...
    def __init__(self, cognitive_state_dim: int = 256, num_neurons: int = 512): # V4 cognitive state dim
        ...
        self.weights = np.random.randn(cognitive_state_dim, num_neurons).astype(np.float32) * 0.01
    ...
    def learn(self, cognitive_state: np.ndarray, learning_rate: float):
        if cognitive_state is None or cognitive_state.size == 0: return
        input_pattern = cognitive_state.reshape(1, -1).astype(np.float32)
    ...
    def vectorized_learn(self, cognitive_states: np.ndarray, learning_rate: float):
        ...
        """
        if cognitive_states is None or cognitive_states.size == 0: return
        ...
        cognitive_states = cognitive_states.astype(np.float32)
    ...
    def recall(self, cognitive_state: np.ndarray) -> np.ndarray:
        if cognitive_state is None or cognitive_state.size == 0:
            return np.zeros((1, self.num_neurons), dtype=np.float32)
        query_pattern = cognitive_state.reshape(1, -1).astype(np.float32)
        if query_pattern.shape[1] != self.cognitive_state_dim:
            ...
            return np.zeros((1, self.num_neurons), dtype=np.float32)
        return np.dot(query_pattern, self.weights).astype(np.float32)
    
    def clear(self):
        self.weights = np.random.randn(self.cognitive_state_dim, self.num_neurons).astype(np.float32) * 0.01
...
class PredictiveEngine:
    ...
    def predict(self, current_state: np.ndarray) -> np.ndarray:
        if current_state is None or current_state.size == 0:
            return np.zeros((1, self.output_state_dim), dtype=np.float32)
        input_for_predictor = current_state.reshape(1, -1).astype(np.float32)
        if input_for_predictor.shape[1] != self.input_state_dim:
            ...
            return np.zeros((1, self.output_state_dim), dtype=np.float32)
        return self.predictor_network.forward(input_for_predictor).astype(np.float32)
...
class MemorySystem:
    ...
    def __init__(self, 
                 sfe_feature_dim: int = 55,       # V4
                 cognitive_state_dim: int = 256,  # V4
                 emotional_state_dim: int = 108   # V4
                 ):
...
if __name__ == "__main__":
    logger.info("MemorySystem module loaded successfully.")