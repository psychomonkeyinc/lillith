...
class Output:
    """
    Lillith's Expression & Control System.
    Translates Lillith's unified cognitive state, emotional state, and internal language
    into precise control parameters for VocalSynth and simulated mouse/keyboard.
    """
    ...
    def __init__(self,
                 ...
                 internal_lang_dim: int = 512,
                 vocal_synth_params_dim: int = 512,
                 attention_focus_dim: int = 512,
        ...
        self.internal_hidden_dim = int(internal_hidden_dim)
    def update_dimensions(self,
        ...
        if attention_focus_dim is not None:
            self.focus_vector_dim = int(attention_focus_dim)
        if internal_hidden_dim is not None:
            self.internal_hidden_dim = int(internal_hidden_dim)
        ...
        self.total_output_params_dim = (
            len(self.VOCAL_SYNTH_PARAMS) +
            self.MOUSE_CONTROL_DIMS +
            self.KEYBOARD_CONTROL_DIMS
        )
    ...
    # (goals_dim already set above)
        ...
        # Output control param dimensionality (fixed small)
        self.total_output_params_dim = (
            len(self.VOCAL_SYNTH_PARAMS) +
            self.MOUSE_CONTROL_DIMS +
            self.KEYBOARD_CONTROL_DIMS
    ...
    def generate_controls(self,
        ...
        mouse_kb_start_index = len(self.VOCAL_SYNTH_PARAMS)
        mouse_params = raw_output_params[mouse_kb_start_index : mouse_kb_start_index + self.MOUSE_CONTROL_DIMS]
        kb_params = raw_output_params[mouse_kb_start_index + self.MOUSE_CONTROL_DIMS:]
        
        control_dict['mouse_x_delta'] = float(mouse_params[0]) # -1 to 1
        control_dict['mouse_y_delta'] = float(mouse_params[1]) # -1 to 1
        control_dict['left_click_pressure'] = float((mouse_params[2] + 1.0) / 2.0) # 0 to 1
        control_dict['right_click_pressure'] = float((mouse_params[3] + 1.0) / 2.0) # 0 to 1
        control_dict['scroll_delta'] = float(mouse_params[4]) # -1 to 1
        
        control_dict['key_press_strength_map'] = ((kb_params + 1.0) / 2.0).tolist() # 0 to 1
        ...
        self._output_confidence = np.clip((language_clarity_proxy * 0.7 + goals_satisfaction_avg * 0.3), 0.0, 1.0)
    ...
    def generate_output(self, unified_cognitive_state: np.ndarray, emotional_state: np.ndarray, focus_vector: np.ndarray) -> Dict[str, Any]:
        ...
        # Reuse generate_controls path (which revalidates dims) by splitting pieces
        try:
...
if __name__ == "__main__":
    logger.info("Output module loaded successfully.")